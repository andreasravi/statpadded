...
- **Output:** `data/receiving_stats.csv` — one row per player-season for
  ...
  `year, player_id, player, team, position, games, targets, receptions,
   receiving_yards, receiving_tds, receiving_air_yards, target_share`

## Fetching (auto)

Run `pipeline.py`. A year with no cache downloads that season's
`stats_player_reg` file once, keeps the receiving columns, and caches the
lean extract at `data/raw/receiving_stats_{year}.csv`. Re-running is
instant.
...
- `team` is `recent_team` (the player's last team that season, so a
  mid-season trade shows only the destination), normalized to this repo's
  canonical abbreviation via `nfl/common/team_codes.normalize_nflverse_abbr`
  (nflverse uses `LA` for the Rams; this repo uses `LAR`).
