"""
Analysis on top of data/rb_prop_grades.csv (RB counterpart of analyze_wr.py):

  1. Season-by-season O/U hit rate (rushing yards / rushing TDs)
  2. Hit rate bucketed by the SIZE of the line, all vs healthy (>=14 G)
  3. Year-over-year: bucket an RB's result vs his line, then his result the
     NEXT year (mean reversion + how the book moves the line)

Only `yards_kind == "rush"` rows feed the yards views (the 8 rush+rec-only
2022 backs are excluded so the yardage scale is consistent).

Writes one CSV per view to data/rb_*.csv and prints a summary.
Run: python3 projects/prop-accuracy/scripts/analyze_rb.py
"""
import csv
import os
import re
...
GRADES = os.path.join(PROJECT, "data", "rb_prop_grades.csv")
...
def load():
    ...
    with open(GRADES) as f:
        for r in csv.DictReader(f):
            for k in ("yards_line", "yards_diff", "td_line", "td_diff"):
...
def by_season(rows):
    ...
    for y in sorted({r["year"] for r in rows}):
        ...
        rec = {"year": y, "n": len(s)}
        for stat in ("yards", "td"):
            g = [r for r in s if r[f"{stat}_diff"] is not None
                 and (stat != "yards" or r["yards_kind"] == "rush")]
            o = sum(1 for r in g if r[f"{stat}_diff"] > 0)
            rec[f"{stat}_over"], rec[f"{stat}_graded"] = o, len(g)
            rec[f"{stat}_over_pct"] = pct(o, len(g))
        out.append(rec)
    _write("rb_hit_rate_by_season.csv",
...
def by_line_size(rows):
    for stat, (edges, labels) in TIERS.items():
        ...
        _write(f"rb_hit_rate_by_{stat}_line.csv",
...
def yoy(rows):
    idx = {(_norm(r["player"]), r["year"]): r for r in rows
           if r["yards_diff"] is not None and r["yards_kind"] == "rush"}
    pairs = []
    ...
    _write("rb_year_over_year_pairs.csv",
    ...
    _write("rb_year_over_year_reversion.csv",
...
def main():
    ...
    print(f"{len(rows)} graded RB-seasons ({yrs[0]}-{yrs[-1]})\n")
    print("Rushing-yards O/U cleared by season (rush-only lines):")
    for s in seasons:
        print(f"  {s['year']}: {s['yards_over']}/{s['yards_graded']} ({s['yards_over_pct']}%)   "
              f"TD {s['td_over']}/{s['td_graded']} ({s['td_over_pct']}%)")

    print("\nRushing-yards O/U by size of the line:")
    ...
    print("\nRushing-TD O/U by size of the line:")
    for t in tiers["td"]:
        print(f"  {t['tier']:<11} {t['n']:>3} {str(t['over_pct_all'])+'%':>5} "
              f"{str(t['over_pct_healthy'])+'%':>8} {str(t['median_diff_healthy']):>12}")
    ...
    if len(pairs) >= 3:
        ...
        sda, sdb = st.pstdev(da), st.pstdev(db)
        if sda and sdb:
            print(f"\ncorr(year-A diff, year-B diff) = {cov/(sda*sdb):+.2f}  (n={n} pairs)")
    print(f"\nwrote 6 view CSVs -> {os.path.relpath(DATA, REPO_ROOT)}/ (rb_*.csv)")
